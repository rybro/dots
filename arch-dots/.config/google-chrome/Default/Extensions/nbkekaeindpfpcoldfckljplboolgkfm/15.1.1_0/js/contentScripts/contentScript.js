(function(){
	...
	// ======================  YOUTUBE  ==============================================================
	function buttonYouTube( mainMedia, msg_ad, port ) {
	
		const BUTTON_ID = "GetThemAll_yt_button";
		...
		function createButtonElement( buttonId ){
			...
			span_title.textContent = chrome.i18n.getMessage("button_download");
		...
		// ------------------------------------	
		function buildMediaList( media, button, msg_ad ){

			var menu = document.getElementById( "fvd-yt-uix-button-menu" );
			if(menu != null)
			{
			...
			for( var k in media )
			{
				...
				img.setAttribute("align","left");
				img.setAttribute("src", imageUrl);
				img.setAttribute("style", "margin-left:-6px;margin-right: 1px;margin-top:3px");
				span.appendChild(img);
		
				var span_t = document.createElement("span");
				span_t.setAttribute( "style", "margin-left: 10px;" );
				span_t.textContent = m.format;
			...
			if( media.length == 0 )
			{
				...
				span.innerHTML = '<img style="margin-left:-6px;margin-right: 1px;margin-top:3px" src="' + EXT_IMAGES['set'] + '" align="left">Setting';	
				li.appendChild( span );	
			...
			if( msg_ad )
			{
				...
				span.innerHTML = '<img style="margin-left:-6px;margin-right: 1px;margin-top:3px" src="' + EXT_IMAGES['AD'] + '" align="left">'+ad.title;	
				li.appendChild( span );	
		...
		if( !document.getElementById( BUTTON_ID ) )
		{
			...
			button.addEventListener("click", function(event){
								...
								chrome.extension.sendRequest({command:"getVideoData"  },	function( media ) { 		
																...
																buildMediaList( media, button, msg_ad );						
								
															}	);

							}, false);
			
			var elemFlag = document.getElementById('watch7-views-info');
			...
			else
			{
				...
				if( !actionsDiv )
				{
					var actionsDiv = document.getElementById('watch7-sentiment-actions');					
				}
		...
		if( document.location.href.indexOf( "youtube.com/user/" ) != -1 )
		{
			if( !document.getElementById( BUTTON_ID ) )
			{
				...
				var elemFlag = document.getElementById( "channel-navigation-menu" );
				...
				{
					var actionsDiv = document.getElementsByClassName('video-content-info')[0];
					if( actionsDiv )
					{
						var div = document.createElement("div");
	...
	function iconVKontakte( media, port ) {
		...
		var inputElems = document.querySelectorAll('input[type="hidden"]');	

		for (var i=0; i<inputElems.length; i++)		{
			(function( inp, name ){
				var fn = getFileName( inp.value );
				if ( fn && fn == name) {	
					add_icon( inp );
				}
			})( inputElems[i], media.fileName );
		}		
		
		// ------------------------------
		function getFileName( url ){
			var tmp = url.split( "?" );
			if ( tmp.length > 0)  url = tmp[0];
			tmp = url.split( "/" );
			if (tmp.length>1) {
		...
		// ------------------------------
		function parent(elem, findClass) {

			var ee = elem;
			for (var i=0;  i<10; i++) {
				var cName = ee.className;
				if (cName.indexOf( findClass ) != -1)  return ee;
				ee = ee.parentNode;
			}
			return null;
		}	
		// ------------------------------
		function add_icon(elems_input) {
			
			var audioBlock = parent(elems_input, 'audio');
			if (!audioBlock) return;
			
			var url_input = elems_input.value;
			var p = url_input.split(",");
			url_input = p[0];
			
			var id_input = elems_input.id;
			var id = id_input.substr(10);
			id = id.replace("-", "");			

			if (url_input == media.url)	 {
 				//var audioBlock = document.getElementById("audio"+id);
				//if (!audioBlock) return;
				var titleBlock = audioBlock.getElementsByClassName("info")[0];
				if (!titleBlock) return;
				var title = titleBlock.getElementsByTagName( "a" )[0].textContent;
				var sps = titleBlock.getElementsByTagName( "span" );
				for (var j=0; j<sps.length; j++)  {
					title += "-" + sps[j].textContent;
				}	
				if (title[title.length-1] == '-')  title = title.substring(0, title.length-2).trim();
				
				media.title = title;
				media.downloadName = title+'.'+media.ext;
				//console.log(url_input, media, media.downloadName);
 
				var elem_play = document.getElementById("play" + id);	
				...
				var dd = document.getElementById("download" + id);	
				if (dd != null) return;
				...
				if (div_info[0])
				{
					div_info[0].setAttribute("style", "width:375px");
				}	
				
 				chrome.extension.sendRequest({	akce:"set_VK_Audio_Media_title",  
												tabId: media.tabId,	
												url: media.url,	
												ext: media.ext,	
												title: title  },	
		...
		port.onMessage.addListener( function( message ){
			...
			switch( message.action ){
				...
				case "insertYTButton":
					...
					var media = message.media;
					var ad = (message.ad ? message.ad : null);
					
					buttonYouTube( media, ad, port );
