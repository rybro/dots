(function(){
	...
	var MediaStorage = function(){
		...
		function itemAlreadyExists( tabId, item ){
			...
			if( data[tabId] )	{
				for( var i = 0; i != data[tabId].length; i++ )	{
					...
					if(EXTENSION_NAME.Media[item.source].isEqualItems( item, existsItem ))			return true;
				}
			}
			return false;
		}
		...
		// ----------------------------------------------------------------------------------------------------
		// заменит SitePage со Sniffer (если совпадают)
		...
		function getDataForHash( hash ){
			...
			for( var tabId in data ){
				data[tabId].forEach( function( item ){
						if ('yt_hash' in item && item.yt_hash == hash)  result = item;
				} );
			}	
...
//						if ( item.type == "video" || item.type == "audio" || item.type == "game")
						if ( item.type == "video" || item.type == "game")	{
		...
		// ----------------------------------------------------------------------------------------------------
		this.setStream = function( yt_hash, status, size ){
		
			for( var tabId in data ){

				for( var i = 0; i != data[tabId].length; i++ ) {

							if ( data[tabId][i].yt_hash == yt_hash )	{
		...
		this.addItemForTab = function( tabId, item ){
			...
			if( itemAlreadyExists_source( tabId, item ) )			return;
