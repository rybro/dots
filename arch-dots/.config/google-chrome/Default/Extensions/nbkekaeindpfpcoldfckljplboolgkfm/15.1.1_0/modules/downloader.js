	...
	(function(){
		...
		var Downloader = function(){		
			...
			this.start = function( params, cb ){
				...
				console.log('Downloader.start', params);
				...
				for (var i=0; i<listMedia.length; i++) {
					...
					var tt = null;
					if (listMedia[i].downloadName)	tt = listMedia[i].downloadName + (listMedia[i].ext ? '.'+listMedia[i].ext : ''); 
					if (!tt && listMedia[i].title)  tt = listMedia[i].title;
					
					queue.push( {	name:	unique_title( tt, listMedia[i].ext ),
									url:	listMedia[i].url,
									ext:	listMedia[i].ext,	
									id:		listMedia[i].id,
			...
			this.stop = function( id, callback ){
				...
				console.log('Downloader.stop', id);
			...
			function create( status, callback ){
				...
				if (statusCreater == 99) return;		// в паузе

				if (status == 1) {		// создаем
					statusCreater = 99;
					createZipWriter( function() {			
						statusCreater = 2;			// готов к созданию архива
						console.log('---createZipWriter---');
						callback();
					});
				}	
				else if (status == 11) {		// закрываем
					check_queue( function(){			// до пишем в архив
						
						zipFiles(function(url) {
							statusCreater = 12;			// завершили
							callback(url);
						});
						
					});
				}
				else if (status == 5) {		// считали файл
